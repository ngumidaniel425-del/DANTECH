...
// 3. Extract Context (Crucial for filtering)
...
if ($type == "register_teacher") {
    ...
    foreach ($students as $s) {
        $q = "INSERT INTO students_master (admission, fullname, class_name, school_name, department) 
              VALUES ($1, $2, $3, $4, $5) ON CONFLICT (admission, school_name) 
    ...
    foreach ($attendance as $row) {
        ...
        if (!pg_query_params($conn, $q, array($row['student_adm'], $row['class_name'], $row['lesson_name'], $row['period_type'], $row['attendance_date'], $row['student_name'], $school, $dept))) $success = false;
    }
    echo json_encode(["status" => $success ? "success" : "error"]);

} elseif ($type == 'fetch_master_data') {
    // RECTIFIED: Filter strictly by Department and School
